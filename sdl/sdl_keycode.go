...
// Keycode is the virtual key representation using the current keyboard layout.
type Keycode uint32

// Keymod represents keyboard modifier state.
type Keymod uint16
...
const (
	...
	KMOD_LEVEL5 Keymod = C.SDL_KMOD_LEVEL5
...
// Keycode bitmask constants used to construct keycodes from scancodes.
const (
	K_SCANCODE_MASK Keycode = C.SDLK_SCANCODE_MASK
	K_EXTENDED_MASK Keycode = C.SDLK_EXTENDED_MASK
)

// ScancodeToKeycode converts a scancode to a keycode.
func ScancodeToKeycode(sc Scancode) Keycode {
	return Keycode(uint32(sc) | uint32(K_SCANCODE_MASK))
}
