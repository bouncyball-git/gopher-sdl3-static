...
extern void goHintCallback(void *userdata, char *name, char *oldValue, char *newValue);

static void cgoHintCallbackTrampoline(void *userdata, const char *name, const char *oldValue, const char *newValue) {
	goHintCallback(userdata, (char *)name, (char *)oldValue, (char *)newValue);
}

static SDL_HintCallback _get_hint_callback_trampoline(void) {
	return cgoHintCallbackTrampoline;
}
*/
import "C"
...
func GetHint(name string) string {
	cn := C.CString(name)
	defer C.free(unsafe.Pointer(cn))
	return C.GoString(C.SDL_GetHint(cn))
}

// GetHintBoolean returns the boolean value of a hint.
func GetHintBoolean(name string, defaultValue bool) bool {
	cn := C.CString(name)
	defer C.free(unsafe.Pointer(cn))
	return bool(C.SDL_GetHintBoolean(cn, C.bool(defaultValue)))
}

// ResetHint resets a hint to its default value.
func ResetHint(name string) bool {
	cn := C.CString(name)
	defer C.free(unsafe.Pointer(cn))
	return bool(C.SDL_ResetHint(cn))
}

// ResetHints resets all hints to their default values.
func ResetHints() {
	C.SDL_ResetHints()
}

// HintPriority represents the priority level for a hint.
type HintPriority int

// Hint priority constants.
const (
	HINT_PRIORITY_DEFAULT  HintPriority = C.SDL_HINT_DEFAULT
	HINT_PRIORITY_NORMAL   HintPriority = C.SDL_HINT_NORMAL
	HINT_PRIORITY_OVERRIDE HintPriority = C.SDL_HINT_OVERRIDE
)
...
func SetHintWithPriority(name, value string, priority HintPriority) bool {
	...
	defer C.free(unsafe.Pointer(cn))
	defer C.free(unsafe.Pointer(cv))
	return bool(C.SDL_SetHintWithPriority(cn, cv, C.SDL_HintPriority(priority)))
}

// HintCallbackFunc is called when a hint value changes.
type HintCallbackFunc func(name, oldValue, newValue string)

//export goHintCallback
func goHintCallback(userdata unsafe.Pointer, name, oldValue, newValue *C.char) {
	id := uintptr(userdata)
	fn := getCallback(id).(HintCallbackFunc)
	var old, nw string
	if oldValue != nil {
		old = C.GoString(oldValue)
	}
	if newValue != nil {
		nw = C.GoString(newValue)
	}
	fn(C.GoString(name), old, nw)
}

// AddHintCallback registers a callback to be called when a hint changes.
// Returns the callback ID for use with RemoveHintCallback.
func AddHintCallback(name string, callback HintCallbackFunc) uintptr {
	id := registerCallback(callback)
	cn := C.CString(name)
	defer C.free(unsafe.Pointer(cn))
	C.SDL_AddHintCallback(cn, C._get_hint_callback_trampoline(), ptrFromID(id))
	return id
}

// RemoveHintCallback removes a previously added hint callback.
func RemoveHintCallback(name string, id uintptr) {
	cn := C.CString(name)
	defer C.free(unsafe.Pointer(cn))
	C.SDL_RemoveHintCallback(cn, C._get_hint_callback_trampoline(), ptrFromID(id))
...
const (
	...
	HINT_OPENXR_LIBRARY = "SDL_OPENXR_LIBRARY"
	...
	HINT_JOYSTICK_DRUM_DEVICES = "SDL_JOYSTICK_DRUM_DEVICES"
	...
	HINT_JOYSTICK_GUITAR_DEVICES = "SDL_JOYSTICK_GUITAR_DEVICES"
	...
	HINT_JOYSTICK_HIDAPI_GAMESIR = "SDL_JOYSTICK_HIDAPI_GAMESIR"
	...
	HINT_WINDOWS_RAW_KEYBOARD_INPUTSINK = "SDL_WINDOWS_RAW_KEYBOARD_INPUTSINK"
